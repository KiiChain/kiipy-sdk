...
class QueryRestClientTestCase(TestCase):
    ...
    @patch("kiipy.common.rest_client.MessageToDict")
    ...
    def test_make_url_helper_method(self, messageToDict_mock):
        ...
        assert (
            client._make_url(base_url)  # noqa: # pylint: disable=protected-access
            == f"{base_address}{base_url}"
        )
        ...
        assert (
            client._make_url(  # noqa: # pylint: disable=protected-access
                base_url, request=request_data
            )
            == "https://base.addr/base_url?key3=12&key2=13&key1=11"
        )
        assert (
            client._make_url(  # noqa: # pylint: disable=protected-access
                base_url, request=request_data, used_params=["key2"]
            )
